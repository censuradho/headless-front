...
export const THEME_KEY = LOCAL_STORAGE_KEY
...
export function ThemeProvider({ children }: ThemeProviderProps) {
  ...
  const [_isDarkThemePreferences, setIsDarkThemePreferences] = useState(
    getIsDarkThemePreferences()
  );
  ...
  const savedTheme =
    themeStorage || (_isDarkThemePreferences ? DARK_THEME : LIGHT_THEME);

  const [currentTheme, setCurrentTheme] = useState(savedTheme);
  const [theme, setTheme] = useState(
    mapTheme[currentTheme as keyof typeof mapTheme]
  );
